...
def sample_task(data_model, raw_class_num, model, query_num_per_class_per_model=1, class_num=5,
    ...
    s['data'], s['label'], s['modal'] = shuffle_datas(s['data'], s['label'], s['modal'])
    q['data'], q['label'], q['modal'] = shuffle_datas(q['data'], q['label'], q['modal'])
...
def make_set_tensor(dict_set):
    ...
    return img_batch, labels.astype(np.float), modals.astype(np.float)
...
def distance(x, y, sw=None):
    ...
    with tf.name_scope("compute_distance"):
        ...
        dim = size*size*FLAGS.filter_num
        ...
        if FLAGS.distance_style == 'euc_v2':
        ...
        elif FLAGS.distance_style == 'euc_norm':
            with tf.name_scope('euc_norm'):
                x_fill_op = tf.ones((1, dim), dtype=tf.float32)
                y_fill_op = tf.ones((1, dim), dtype=tf.float32)
                x_norm = x / tf.matmul(tf.reshape(tf.sqrt(tf.reduce_sum(x * x, axis=1)), (-1, 1)), x_fill_op)
                y_norm = y / tf.matmul(tf.reshape(tf.sqrt(tf.reduce_sum(y * y, axis=1)), (-1, 1)), y_fill_op)
                x_tile = tf.reshape(tf.tile(x_norm, [1, width]), (high, width, dim))
                y_tile = tf.tile(input=tf.reshape(y_norm, (1, -1, dim)), multiples=[high, 1, 1])
                distance = tf.sqrt(tf.reduce_sum(tf.square(x_tile - y_tile), axis=2))
            ...
            raise AttributeError("set error distance style.")
...
def get_prototype(support_x, s_label):
    ...
    return prototypes
def proto_various_loss(support_x, support_y, support_m):
    with tf.name_scope("proto_various"):
        prototypes = get_prototype(support_x, support_y)
        modal_prototypes, _, _ = support_label_modal_proto(support_x, support_y, support_m)
        modal_prototypes = tf.reshape(modal_prototypes, (FLAGS.model, FLAGS.way_num, -1))
        intra_variance = tf.math.reduce_variance(prototypes, axis=0)
        inter_variance = tf.reduce_mean(tf.math.reduce_variance(modal_prototypes, axis=0), axis=0)
        var_loss = tf.reduce_mean(inter_variance/intra_variance)
    return var_loss
